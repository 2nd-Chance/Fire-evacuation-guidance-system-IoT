...
static GenericResource _generic_resource;
...
static void _duplicate_string(char **target_string, string source_string)
{
	try {
		*target_string = new char[source_string.length() + 1];
		snprintf(*target_string, (source_string.length() + 1), "%s", source_string.c_str());
	} catch (bad_alloc& exception) {
		cerr << "Bad allocation in target_string " << exception.what() << endl;
		return ;
	}
}
...
static OCStackResult _set_device_info(void)
{
	...
	if(result != OC_STACK_OK) {
		cout << "Failed to set protocol independent id" << endl;
		return result;
	}
...
int main(void)
{
	...
	try {
		_generic_resource.CreateResource();
